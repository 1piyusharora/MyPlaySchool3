...
public class ViewEnquiryDetailActivity extends AppCompatActivity {
    ...
    void initview(){
        ...
        tvRegisterStu.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(studentBean.getEnqStatus()==2||studentBean.getEnqStatus()==1) {
        ...
        tvUpdateVisit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                EnquiryDescription=studentBean.getEnqdesc();
                if(studentBean.getEnqStatus()==2||studentBean.getEnqStatus()==1) {
                    AlertDialog.Builder dialog =
                            new AlertDialog.Builder(ViewEnquiryDetailActivity.this);
                    dialog.setMessage("Enquiry is Processed");
    ...
    void updateNextVisitDialog(){
        ...
        edtNextDate.setText(studentBean.getNextVisit());
    ...
    void listParentPhoneNumbers(int signal,final int callMsg){
        if(signal==0){
            if(studentBean.getFather_phone().toString().length()==0)
                Toast.makeText(this, "No contact details", Toast.LENGTH_SHORT).show();
            else{
                android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(ViewEnquiryDetailActivity.this);
                String[] options = studentBean.getFather_phone().split(",");
    ...
    void setData(){
        ...
        int number=studentBean.getEnqid();
        tvManageEnquiryStuName.setText(studentBean.getName());
        String str="";
        //str = str+"Enquiry Number:"+number;
        if(studentBean.getDob().toString().contains("0000-00-00"))
            str=str+"Date of Birth: NA";
        else
            str=str+"Date of Birth: "+studentBean.getDob();
        ...
        if(studentBean.getFather_name().length()>0)
            str = str+"\nFather's Name: "+studentBean.getFather_name();
        else
            str = str+"\nFather's Name: NA";

        try {
            if(studentBean.getFather_qualification().length()>0)
                str = str+"\nFather's Qualification: "+studentBean.getFather_qualification();
            else
        ...
        try {
            if(studentBean.getFather_occupation().length()>0)
                str = str+"\nFather's Occupation: "+studentBean.getFather_occupation();
            else
        ...
        try {
            if(studentBean.getFather_company_name().length()>0)
                str = str+"\nFather's Company Name: "+studentBean.getFather_company_name();
            else
        ...
        try {
            if(studentBean.getFather_phone().length()>0)
                str = str+"\nFather's Phone Number: "+studentBean.getFather_phone();
            else
        ...
        if(studentBean.getFather_email().length()>0)
            str = str+"\nFather's E-mail: "+studentBean.getFather_email();
        else
            str = str+"\nFather's E-mail: NA";

/*        if(studentBean.getMother_name().length()>0)
            str = str+"\nMother's Name: "+studentBean.getMother_name();
        else
            str = str+"\nMother's Name: NA";*/
       ...
       /* try {
        ...
        if(studentBean.getEnqStatus()==1){
            str = "Registered";
        }else if(studentBean.getEnqStatus() == 0){
            if(studentBean.getUserType() == 3){
                str = "Demo";
            }else if(studentBean.getUserType() == 0){
                str = "Enquiry";
            }else if(studentBean.getUserType() == 2){
                str = "Counselling";
            }
        }else if(studentBean.getEnqStatus() == 2){
            str = "Discared";
        }
        String str1="";
        str1="Course Name: "+studentBean.getCourseName();

        if(studentBean.getNextVisit().equals("0000-00-00"))
            str1 = str1+"\nNext Follow Up: NA\nStatus: "+str;
        else
            str1 = str1+"\nNext Follow Up: "+getFormattedDate(studentBean.getNextVisit())+"\nStatus: "+str;

        str1=str1+"\nReference: "+studentBean.getReference();
//        if(studentBean.getReferenceBy().length()>0)
//            str1=str1+"\nReference by: "+studentBean.getReferenceBy();
        if(studentBean.getEnqdesc().toString().contains(",")){
            String[] str2 = studentBean.getEnqdesc().split(",");
            ...
            str1 = str1+"\nComment: "+studentBean.getEnqdesc();
        }
