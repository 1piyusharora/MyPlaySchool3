...
public class EduEnquiryAdapter extends ArrayAdapter {
    ...
    public void filter(String search) {
        if (search.isEmpty()) {
            ...
            for (int i = 0; i < searchlist.size(); i++) {
                if (searchlist.get(i).getName().toLowerCase().contains(search.toLowerCase()) ||
                        searchlist.get(i).getCourseName().toLowerCase().contains(search.toLowerCase())) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
      ...
      textView2.setText(AttUtil.getFormattedDate( bean.getDate()) + " - " + AttUtil.getFormattedTime(bean.getTime()));
        //if(bean.getCourseid()==1){
        textView1.setText(bean.getCourseName());
        int number = bean.getEnqid();
